...
from datasets.kinetics400 import Kinetics400SRSCDataset
...
def parse_args():
    ...
    parser.add_argument('--mask', default=False, action='store_true', help='whether to use mask in PointerNet')
...
if __name__ == '__main__':
    ...
    srsc = SRSC(base_network=base, feature_size=512, tuple_len=args.tl,
                hidden_dim=512, input_dim=512, mask=args.mask, p=0.5).to(device)
    ...
    if args.mode == 'train':  ########### Train #############
        ...
        train_dataset = Kinetics400SRSCDataset('data/compress', args.cl, args.it, args.tl, True, train_transforms)
        nums = len(train_dataset)
        train_dataset, val_dataset = random_split(train_dataset, (int(nums*0.95), nums-int(nums*0.95)))
        ...
        base_network_groups = {'params': [p for n,p in srsc.named_parameters() if 'base_network' in n], 'lr': args.lr, 'weight_decay':5e-4} 
