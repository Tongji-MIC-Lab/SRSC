...
class SRDAblateHAP(nn.Module):
	def __init__(self, input_dim, hidden_dim, dropout, tuple_len):
		super(SRDAblateHAP, self).__init__()
		...
		self.conv1d = nn.Conv1d(tuple_len, 1, 1)
		self.v = nn.Linear(hidden_dim, tuple_len)

	def forward(self, inps, enc_output):
		# inps size bs * T * input_dim
		inp0 = self.conv1d(enc_output)

		inp = torch.cat([inp0, inps], dim=1)[:,:-1,:] # bs * T * hidden_dim
		d, (h, c) = self.lstm(inp) # d size (bs, T, hidden_dim)
		probs = self.v(d) # prob_logits size (bs, T, n)
		_, ptrs = torch.max(probs, dim=2) # ptrs size (bs, T)

		return probs, ptrs
		

class TaskNetAblateOAE(nn.Module):
	def __init__(self, input_dim, hidden_dim, dropout, num_heads=4, tuple_len=3):
		super(TaskNetAblateOAE, self).__init__()
...
class TaskNetAblateHAP(nn.Module):
	def __init__(self, input_dim, hidden_dim, dropout, num_heads=4, tuple_len=3):
		super(TaskNetAblateHAP, self).__init__()
		...
		self.SRD = SRDAblateHAP(input_dim, hidden_dim, dropout, tuple_len) 
