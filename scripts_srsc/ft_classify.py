...
def load_pretrained_weights(ckpt_path):
    ...
    for name, params in pretrained_weights.items():
        if 'base_network' in name:
            name = name[name.find('base_network')+len('base_network')+1:]
        else:
...
def parse_args():
    parser = argparse.ArgumentParser(description='Finetune 3D CNN from SRSC pretrained weights')
    ...
    parser.add_argument('--ft-mode', type=str, default='part')
    # parser.add_argument('--ft-mode', default=False, action='store_true')
...
if __name__ == '__main__':
    ...
    for data in train_dataloader:
        clips, idxs = data
        writer.add_video('train/clips', clips.permute(0,2,1,3,4), 0, fps=8)
