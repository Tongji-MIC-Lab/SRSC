...
class CaptionModel(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.ssfeat_dims = opt.ssfeat_dims
        ...
        # ss_feat part
        self.fusion_linear = nn.Linear(self.ssfeat_dims, self.feat_dims[1]) # index 1 for 3D features
        self.fusion_activation = nn.ReLU()
        # for r(2+1)d decompose fusion
        self.sampled_idxs = np.linspace(0, self.feat_dims[1]-1, self.feat_dims[0])
    ...
    def forward(self, feats, bfeats, ssfeats, seq, pos):
        ssfeats = self.fusion_activation(self.fusion_linear(ssfeats))
        feats[1] = feats[1] + 0.25*ssfeats
        feats[0] = feats[0] + 0.10*ssfeats[...,self.sampled_idxs]
    ...
    def sample(self, feats, bfeats, ssfeats, pos, opt={}):
