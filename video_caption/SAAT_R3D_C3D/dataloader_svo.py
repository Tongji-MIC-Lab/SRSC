...
class DataLoader():
	...
	def __init__(self, opt):
		...
		# ss features directory
		self.ss_feat_dir = opt.get('ss_feat_dir')
		self.id2vid = opt.get('id2vid')
	...
	def get_batch(self):
		...
		for ii in range(self.batch_size):
			...
			if 'YouTubeClips' in self.ss_feat_dir:
				load_path = os.path.join(self.ss_feat_dir, self.id2vid['vid'+str(video_id)]+'.pt')
			else: # msrvtt
				load_path = os.path.join(self.ss_feat_dir, self.id2vid[video_id]+'.pt')

			ss_feat = torch.load(load_path)
			# import pdb; pdb.set_trace()
			# ss_feat_len = len(ss_feat)
			# ss_sampled_idxs = np.linspace(0, ss_feat_len-1, 25, dtype=int) 
			# ss_feat = ss_feat[ss_sampled_idxs]
			ss_batch.append(ss_feat.mean(0).view(1, 1, -1))
			# (1, 1, 512)
		...
		data['ssfeats'] = torch.cat(ss_batch, dim=0)
