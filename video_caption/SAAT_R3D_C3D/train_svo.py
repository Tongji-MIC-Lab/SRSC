...
def train(
    ...
    while True:
        ...
        if rl_training:
            ...
            pred, _, _, pred_svo, _, _ = model(feats, bfeats, ssfeats, labels, labels_svo)
...
def validate(model, criterion, loader, opt):
    ...
    for ii in range(num_iters):
        ...
        bfeats = data['bfeats']	
        ssfeats = data['ssfeats']
        # logger.info(len(feats))
        # logger.info(feats[0].size())
        # logger.info(feats[1].size())
        # logger.info(len(bfeats))
        # logger.info(bfeats[0].size())
        # logger.info(bfeats[1].size())	
        if loader.has_label:
            labels = data['labels']
            masks = data['masks']
            labels_svo = data['labels_svo']
        ...
        if ii == (num_iters - 1) and last_batch_size > 0:
            ...
            ssfeats = ssfeats[:last_batch_size]
            if loader.has_label:
                labels = labels[
                    :last_batch_size *
        ...
        if loader.has_label:
            ...
            # logger.info(labels_svo.size())

            pred, gt_seq, gt_logseq, _, _, _ = model(feats, bfeats, ssfeats, labels, labels_svo)
            #memReport()
        ...
        seq, logseq, _, seq_svo = model.sample(feats, bfeats, ssfeats, labels_svo, {'beam_size': opt.beam_size})
...
if __name__ == '__main__':
    ...
    assert opt.ss_backbone in opt.ss_feat_dir
    # add opt.id2vid
    if 'yt2t' in opt.train_label_h5:
        f = 'Vid2Url_Full.txt'
        msvd_vid2id = eval(open(f, 'r').read())
        print(f'len of msvd_vid2id dict: {len(msvd_vid2id)}') 
        opt.id2vid = deepcopy(msvd_vid2id)
    else: # msrvtt
        opt.id2vid = {i: f'video{str(i)}' for i in range(10000)}
