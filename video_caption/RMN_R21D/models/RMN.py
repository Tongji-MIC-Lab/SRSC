...
class Encoder(nn.Module):
    def __init__(self, opt):
        ...
        # self-supervised feature embedding 
        self.ss_feature_embed = nn.Linear(opt.ss_feature_size, opt.m_feature_size)
        nn.init.xavier_normal_(self.ss_feature_embed.weight)
        self.relu = nn.ReLU()
        self.sampled_idxs = np.linspace(0, opt.m_feature_size-1, opt.a_feature_size, dtype=int)
    ...
    def forward(self, cnn_feats, region_feats, spatial_feats, ss_feats):
        '''
        ...
        # ss features
        embedded_ss_feats = self.relu(self.ss_feature_embed(ss_feats))
        i3d_feats = i3d_feats + 0.25*embedded_ss_feats
        # for 2d 
        frame_feats = frame_feats + 0.10*embedded_ss_feats[:, :, self.sampled_idxs]
...
class CapModel(nn.Module):
    ...
    def forward(self, cnn_feats, region_feats, spatial_feats, ss_feats, captions, teacher_forcing_ratio=1.0):
        frame_feats, i3d_feats, object_feats = self.encoder(cnn_feats, region_feats, spatial_feats, ss_feats)
