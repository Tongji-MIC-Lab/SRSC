...
def evaluate(opt, net, eval_range, prediction_txt_path, reference):
    eval_loader = get_eval_loader(eval_range, opt.feature_h5_path, opt.region_feature_h5_path, 
                                opt.ss_feat_dir, opt.test_batch_size)

    result = {}
    for i, (frames, regions, spatials, ss_feats, video_ids) in tqdm(enumerate(eval_loader)):
        frames = frames.to(DEVICE)
        regions = regions.to(DEVICE)
        spatials = spatials.to(DEVICE)
        ss_feats = ss_feats.to(DEVICE)
  
        outputs, _ = net(frames, regions, spatials, ss_feats, None)
