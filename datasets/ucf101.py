...
class UCF101ClipRetrievalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if 'v_HandstandPushups' in filename:
            filename = filename.replace('v_HandstandPushups', 'v_HandStandPushups')
            # npy_videoname = npy_videoname.replace('v_HandstandPushups', 'v_HandStandPushups')
        videodata = skvideo.io.vread(filename)
        length, height, width, channel = videodata.shape
...
class UCF101SRSCDataset(Dataset):
    """UCF101 dataset for serial restoration of shuffled clips. Generate clips and permutes them on-the-fly.
    ...
    def __getitem__(self, idx):
        ...
        filename = os.path.join(self.root_dir, 'video', videoname)
        npy_videoname = os.path.join(self.root_dir, 'numpy', videoname[:-4]) + '.npy'
        if 'v_HandstandPushups' in filename:
            filename = filename.replace('v_HandstandPushups', 'v_HandStandPushups')
            npy_videoname = npy_videoname.replace('v_HandstandPushups', 'v_HandStandPushups')

        # if os.path.exists(npy_videoname) and False:
        #     videodata = np.load(npy_videoname)
        # else:  
        #     videodata = skvideo.io.vread(filename)
        #     if not os.path.exists(os.path.dirname(npy_videoname)):
        #         os.makedirs(os.path.dirname(npy_videoname))
        #     np.save(npy_videoname, videodata)
        videodata = skvideo.io.vread(filename)
...
def gen_ucf101_srsc_splits(root_dir, clip_len, interval, tuple_len):
    """Generate split files for different configs."""
    srsc_train_split_namesrsc_train_{}_{}_{}.txt'.format(clip_len, interval, tuple_len)
    srsc_train_split_path = os.path.join(root_dir, 'split', srsc_train_split_name)
    srsc_test_split_name = 'srsc_test_{}_{}_{}.txt'.format(clip_len, interval, tuple_len)
    srsc_test_split_path = os.path.join(root_dir, 'split', srsc_test_split_name)
    ...
    test_split.to_csv(srsc_test_split_path, index=None)
...
if __name__ == '__main__':
    ...
    gen_ucf101_srsc_splits('../data/ucf101', 16, 8, 3)
